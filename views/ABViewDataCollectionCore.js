...
module.exports = class ABViewDataCollectionCore extends ABView {
    ...
    fromValues(values) {
        ...
        this.settings.object =
            this.settings.object || ABViewPropertyDefaults.object;
        this.settings.objectUrl =
            this.settings.objectUrl || ABViewPropertyDefaults.objectUrl;
        this.settings.objectWorkspace = this.settings.objectWorkspace || {
            filterConditions:
                ABViewPropertyDefaults.objectWorkspace.filterConditions,
            sortFields: ABViewPropertyDefaults.objectWorkspace.sortFields
        };

        // Convert to boolean
        this.settings.loadAll = JSON.parse(
            this.settings.loadAll || ABViewPropertyDefaults.loadAll
        );
        this.settings.isQuery = JSON.parse(
            this.settings.isQuery || ABViewPropertyDefaults.isQuery
        );
    }
    ...
    removeField(field, cb) {
        ...
        if (
            ...
            this.settings.objectWorkspace.filterConditions.rules.find(
                (o, i) => {
                    if (o.key === field.id) {
                        // if found splice from array
                        this.settings.objectWorkspace.filterConditions.rules.splice(
                            i,
                            1
                        );
        ...
        if (
            ...
            this.settings.objectWorkspace.sortFields.length
        ) {
        ...
        if (this.datasource) {
            if (
                this.application.objects(
                    (obj) => obj.id == this.datasource.id
                )[0]
            )
                return "object";
            else if (
                this.application.queries((q) => q.id == this.datasource.id)[0]
            )
                return "query";
            else return "";
        } else {
    ...
    init() {
        ...
        let isRelated = (relateData, rowId, PK = "id") => {
            if (Array.isArray(relateData)) {
                return (
                    relateData.filter((v) => (v[PK] || v) == rowId).length > 0
                );
            } else {
        ...
        this.on("ab.datacollection.create", (msg, data) => {
            ...
            if (obj.id == data.objectId) {
                ...
                if (!this.__filterComponent.isValid(values)) return;
    ...
    get dataCollectionLink() {
        ...
        return this.application.datacollectionByID(
            this.settings.linkDataCollection
        );
    }
    ...
    clone(settings, ABViewDataCollection) {
        ...
        var clonedDataCollection = new ABViewDataCollection(
            settings,
            this.application,
            this.parent
        );
    ...
    setCursor(rowId) {
        ...
        var dc = this.__dataCollection;
        if (dc) {
            if (dc.getCursor() != rowId) dc.setCursor(rowId);
                            ...
                            modelLocal
                                ...
                                .then((entries) => {
                                    ...
                                    entries.forEach((entry) => {
                                        ...
                                        if (
                                            this.__filterComponent.isValid(
                                                entry
                                            )
                                        ) {
    ...
    isParentFilterValid(rowData) {
        ...
        if (linkVal.filter) {
            return (
                linkVal.filter((val) => (val.id || val) == linkCursor.id)
                    .length > 0
            );
        } else {
    ...
    currentUserUsername() {
        console.error(
            "!!! ABViewDataCollectionCore.currentUserUsername() not implemented on platform."
        );
        return "spanky";
    }
    ...
    setStaticCursor() {
        if (this.settings.fixSelect) {
            ...
            if (this.settings.fixSelect == "_CurrentUser") {
                ...
                if (row) this.__dataCollection.setCursor(row.id);
            } else if (
                ...
                if (rowId) this.__dataCollection.setCursor(rowId);
            } else {
